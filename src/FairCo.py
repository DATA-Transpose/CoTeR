...
# from ..FDNaive import nueral_torch as NUT
import nueral as NU
...
training_set = {
    ...
    'loss': 'unbiased_loss',
    'hidden_layers': [
        {'input': 150, 'output': 64, 'activation': 'relu'}
    ],
    'batch_size': 400,
}
...
def main(
    ...
    for iter in tqdm(range(iterations)):
        ...
        if iter < 100:
            ...
            if 'FairCo' in ranking_method:
                fairness_error = DSIM.get_unfairness(cum_exposure, runtime_relevances / count, G_in_iter, error=True)

                if lamb is not None:
                    ranking = DSIM.neural_rank(net, predict_x, e_p=fairness_error, KP=lamb)
                else:
                    ranking = DSIM.neural_rank(net, predict_x, e_p=fairness_error)
        ...
        if iter == 99:
            ...
            if ('LTR_Skyline' in ranking_method):
                ...
                train_y = np.array(train_y)
                net = NU.MLP(train_x.shape[1], 64, 1)
            else:
                train_relevance = w_pophist[:iter + 1] - np.concatenate((np.zeros((1, len(docs))), w_pophist[:iter]))
                train_y = [train_relevance[ui, di] for ui in range(len(users)) for di in range(len(docs))]
                train_y = np.array(train_y)
                net = NU.MLP(train_x.shape[1], 64, 1)
            net.train(train_x, train_y, epochs=1000, trial=iter)
            ...
            net.train(train_x, train_y, epochs=20, trial=iter)
